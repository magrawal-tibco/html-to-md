...
| ~~Case normalization mismatch in CSH link lookup.~~ **[NOT AN ISSUE]** CSH `Name` identifiers (e.g. `JNDIConnection`, `WSSAuthentication`) are case-significant by design — they are authored that way in MadCap and consumed case-sensitively by the application's help system. The fallback lowercase lookup is a no-op in practice because the primary exact-case lookup already handles all real cases. No fix needed. | `04_build_csh_maps.py:152`, `199` | ~~Moderate~~ |
...
| Finding | Location | Severity |
|---|---|---|
| **[FIXED]** `load_settings` called `Path.read_text()` and `yaml.safe_load()` with no error handling. Missing config or malformed YAML propagated as uncaught exception in every pipeline script. Fixed in `lib/io_utils.py`: `FileNotFoundError` and `yaml.YAMLError` are now caught and re-raised with a clear message — all 13+ scripts benefit automatically via the shared import. | `lib/io_utils.py:23–24` | Moderate |
...
| **[FIXED]** Image downloads held the page's semaphore slot for all sequential image fetches, blocking the slot from the next page. Restructured `fetch_page` so the semaphore is released after the page download; each image then re-acquires the semaphore independently. Concurrency is correctly bounded and page throughput is no longer gated by image download time. | `02_download.py:200–212` | Moderate |
...
| **[FIXED]** Phase 6 (Java API URL patching) file I/O in `ebx_addon_restructure.py` had no error handling. A single unreadable or unwritable file aborted the entire phase with no count of files successfully patched. Fixed: each `read_text`/`write_text` is now guarded with `except OSError`; errors are counted and reported; the phase continues processing remaining files. | `ebx_addon_restructure.py:351–357` | Moderate |
| **[FIXED]** `preflight_scan` in `ebx_addon_restructure.py` silently skipped unreadable files (`except Exception: continue`), making cross-link scan results silently incomplete. Fixed: now catches `OSError` specifically and prints a warning to stderr, making unreadable files visible. | `ebx_addon_restructure.py:127` | Moderate |
| **[FIXED]** Script 08 Phase 4 cache-miss exited `0`. If the cache source directory didn't exist, a warning was printed but `main()` returned `0` because `errors` only counted webhelp copy failures. Fixed: cache-miss now increments `errors` (and prints to stderr), so `main()` returns `1` when Phase 4 is skipped due to a missing cache. | `08_restructure_ebx.py:406–408` | Moderate |
| **[FIXED]** `copy_assets.py:main()` always returned `0` regardless of failures in `copy_asset_folder`, `save_slug_mappings`, or missing versions. Fixed: each `copy_asset_folder` call is now wrapped in `try/except`; `save_slug_mappings` is guarded separately; `main()` returns `1` if any error occurred. | `copy_assets.py:120` | Moderate |
...
| Finding | Location | Severity |
|---|---|---|
| **[FIXED]** Image downloads held the page's semaphore slot while downloading all images sequentially, blocking the slot from the next page and reducing throughput. Fixed: semaphore is released after page download; each image re-acquires it independently. See §4 for details. | `02_download.py:200–212` | Moderate |
| **[FIXED]** Unconditional HEAD request for `zip_last_modified` fired for every new-format product URL even when `--delta` was not set, wasting a network round-trip per product. Fixed: `_get_zip_last_modified` is now called only when `delta=True`; otherwise `zip_last_modified` is stored as `""`. | `01_build_manifest.py:266` | Moderate |
| Double `rglob("*")` traversal in script 07. `build_path_mapping` and `build_javadocs_mapping` each walk the full source tree independently. | `07_restructure_ebx_addon.py:63`, `93` | Minor |
...
| **[FIXED]** TOC reconstruction (the most complex part of the pipeline) had no tests. Added `tests/test_toc.py` (19 tests) covering `insert_into_tree` (single-segment, deep-nesting, shared-parent reuse, collision warning, same-file no-warn, insertion order), `version_html_root` (MadCap, EBX-with-lang, EBX-without-flag, backslash normalization), and `dir_fallback` logic (single/two-segment paths, majority-vote, empty input, whitespace filtering). All 89 tests pass. | Moderate |
